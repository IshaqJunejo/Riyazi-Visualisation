...
let factorY = canvas_1.height / 1.5;
...
function updateValues()
{
    ...
    output_2.innerHTML = Math.round(Math.cos(angle * Math.PI / 180) * 1000) / 1000;
...
function drawGrid()
{
    ...
    {
        if (i != 1.5)
            ctx_1.fillText((i - 1.5) * -1, 12 + 6, i * factorY / 2 + 4);
    }
}
...
function drawWave()
{
    ...
    {
        let y0 = Math.cos((i -1) * Math.PI / 180) * factorY * -0.5 + canvas_1.height / 2;
        let y1 = Math.cos(i * Math.PI / 180) * factorY * -0.5 + canvas_1.height / 2;
